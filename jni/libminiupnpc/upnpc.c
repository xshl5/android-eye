/* $Id: upnpc.c,v 1.103 2014/05/15 08:41:44 nanard Exp $ */
/* Project : miniupnp
                   ...
                   /*out*/char* ext_ipaddr)
{
    ...
    {
        if(ext_ipaddr != NULL)
            strcpy(ext_ipaddr, externalIPAddress);
...
int main(void)
{
    int ret = -1;
    char ext_ip[40] = {0};
    int pri_port = 8888, ext_port = 40711;

    const char* ext_port_and_protocols[] = {"40711", "tcp"};

    // ret = xshl5_redirect(pri_port, ext_port, NULL, NULL, 0, "eth0", "hahahaha", ext_ip);
    ret = xshl5_redirect_remove(ext_port_and_protocols, 1, 0, "eth0");
    if(ret == 0)
    {
        printf("=============== http://%s:%d\n", ext_ip, ext_port);
    }
